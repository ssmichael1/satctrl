...
pub struct ITRFCoord(pub(crate) Vector3);
...
impl std::convert::From<&[f64]> for ITRFCoord {
    fn from(v: &[f64]) -> Self {
        assert!(v.len() == 3);
        ITRFCoord(Vector3::from_slice(v))
    }
}
...
impl ITRFCoord {
    ...
    ///
    ///
    pub fn from_vector(v: &Vector3) -> ITRFCoord {
        ITRFCoord(*v)
    }
    ...
    /// ```
    ///
    pub fn from_slice(v: &[f64]) -> crate::SCResult<ITRFCoord> {
        //if v.len() != 3 {
        //    return crate::scerror!("Input slice must have 3 elements");
        // }
        Ok(ITRFCoord(Vector3::from_slice(v)))
    }
    ...
    pub fn to_geodetic_rad(&self) -> (f64, f64, f64) {
        ...
        let rho = (self.0[0] * self.0[0] + self.0[1] * self.0[1]).sqrt();
        let mut beta: f64 = f64::atan2(self.0[2], (1.0 - WGS84_F) * rho);
        let mut sinbeta: f64 = beta.sin();
        let mut cosbeta: f64 = beta.cos();
        let mut phi: f64 = f64::atan2(
            self.0[2] + B * EP2 * sinbeta.powf(3.0),
        ...
        let lat: f64 = phi;
        let lon: f64 = f64::atan2(self.0[1], self.0[0]);
        let sinphi: f64 = phi.sin();
        let n: f64 = WGS84_A / (1.0 - E2 * sinphi * sinphi).sqrt();
        let h = rho * phi.cos() + (self.0[2] + E2 * n * sinphi) * sinphi - n;
        (lat, lon, h)
    }
    ...
    pub fn q_ned2itrf(&self) -> Quaternion {
        ...
        Quaternion::from_axis_angle(&Vector3::zhat(), lon)
            * Quaternion::from_axis_angle(&Vector3::yhat(), -lat - PI / 2.0)
    }
    ...
    /// ```
    ///
    pub fn to_ned(&self, ref_coord: &ITRFCoord) -> Vector3 {
        self.q_ned2itrf().conjugate() * (self.0 - ref_coord.0)
    }
    ...
    pub fn q_enu2itrf(&self) -> Quaternion {
        let (lat, lon, _) = self.to_geodetic_rad();
        Quaternion::from_axis_angle(&Vector3::zhat(), lon + PI / 2.0)
            * Quaternion::from_axis_angle(&Vector3::xhat(), PI / 2.0 - lat)
    }
    ...
    /// ```
    ///
    pub fn to_enu(&self, other: &ITRFCoord) -> Vector3 {
        self.q_enu2itrf().conjugate() * (self.0 - other.0)
    }
}
...
mod tests {
    ...
    fn test_ned_enu() {
        ...
        */
        ...
        let itrf1 = ITRFCoord::from_geodetic_deg(lat_deg, lon_deg, hae);
        let itrf2: ITRFCoord = itrf1 + itrf1.q_ned2itrf() * Vector3::from([0.0, 0.0, 10000.0]);
